...
for gamma in Array(gammas)
    ...
    local expected_return = sum(((stocks[week, i]-stocks[week-1, i])/stocks[week-1, i]) * parts[i] for i in 1:num_stocks, week in 2:num_weeks)*100/(num_weeks-1)
    risk = sum(parts[i]*sigma[i,j]*parts[j] for i in 1:(num_stocks-1), j in 1:(num_stocks-1))
